...
**Free-threading lab** now also uses a verified local Qwen build.
...
The two remaining GPT-6 Astra build-model identities (original Iris and MILP) were checked against their original run headers. The public apps do not call their build provider when visitors use them.
